  ...
  <div class="ready">
    ...
    {:else if screen.kind === 'present'}
      ...
      <!--
        ...
        документах есть что сказать, фраза под ним. Класс `.markup` отделяет
        разметку от итогов выше, и на него опираются тесты. Веток у счетчика
        две – «нет ни одной» и «столько-то из стольких-то».
        ...
        Вторая строка `.spread` называет, сколько документов насчитывает
        полностью строгий разбор, и есть только там, где манифест несет эту
        вторую половину: пакет, собранный до 10.09.2026, ее не несет.
      -->
      {#if screen.markup}
        <div class="markup">
          ...
          {:else}
            ...
            {#if screen.markup.beyond_this_parser > 0}
              <p class="spread">
                <span>
                  Полностью строгий разбор насчитывает
                  <span class="count">{spaced(screen.markup.objected_to)}</span>
                </span>
              </p>
            {/if}
            <p class="markup-about">
              Дело в разметке самих документов, а не в сборке. Наш разборщик их не читает: не
              сходятся теги, пропущен знак «=» в атрибуте, значение атрибута не закрыто до конца
              файла.
              {#if screen.markup.beyond_this_parser > 0}Еще
                {spaced(screen.markup.beyond_this_parser)} документов он читает, а полностью строгий –
                нет: внутри значения атрибута стоит голый знак «&lt;» или у имени вида &lt;AO:-…&gt; нет
                части после двоеточия.{/if}
              Все они лежат в пакете вместе с остальными и открываются как обычно – такая разметка мешает
              переводу в PDF, а не хранению. Поименный список с причинами и номерами строк – в файле manifest.json,
              раздел xml.
            </p>
          {/if}
        </div>
