<script lang="ts">
   ...
   *
   * О прежнем пакете сказано условно – «если он был», – и это не
   * осторожность слога. Был ли в выбранной папке пакет, окно не знает:
   * собрать можно в любую папку через «Собрать в папку…», и первая сборка
   * туда прежнего пакета не застает. Фраза, зависящая от состояния, которого
   * окно не видит, была бы ложью в каждом таком случае, а условная верна
   * всегда (заслон выпуска 26.09.2026).
   *
  ...
  const CANCELLED: Record<string, string | undefined> = {
    ...
    exporting:
      'Остановлено во время записи пакета. Незаконченное убрано, а прежний пакет, если он был, остался на месте.',
    validating:
      'Остановлено во время проверки записанного. Незаконченное убрано, а прежний пакет, если он был, остался на месте.',
    publishing:
      'Остановлено перед самой заменой пакета. Прежний пакет, если он был, остался на месте.',
  }
   ...
   *
   * У `collision`, `folder_collision` и `archive_duplicate` под этой фразой
   * печатается вторая строка с `message` ядра, и только у них: там фраза ядра
   * несет данные, которых на экране больше нет нигде – имена двух
   * столкнувшихся документов, имена двух групп CTH с первым документом каждой
   * и имя записи архива. Без них человеку нечего искать в корпусе, а сама
   * фраза ядра про эти три кода ничего сверх имен и не сообщает.
   *
  ...
  const FAILED: Record<string, string | undefined> = {
    ...
    folder_collision:
      'Две группы CTH записаны по-разному, а на диске легли бы в одну папку. Это расхождение в исходных данных, а не сбой сборки.',
    distorted:
  ...
  /**
   * Три кода, у которых под основной фразой стоит вторая строка. Набор, а не
   * проверка на три равенства в разметке: почему этих кодов ровно три,
   * объяснено над `FAILED`, и место объяснения должно быть одно.
   */
  const DETAILED: ReadonlySet<string> = new Set([
    'collision',
    'folder_collision',
    'archive_duplicate',
  ])
...
<main>
  <div class="ready">
    ...
    {:else if screen.kind === 'failed'}
      ...
      <!--
        Вторая строка – фраза ядра целиком, и стоит она у трех кодов из
