<script lang="ts">
   ...
   *
   * У `message` ядра одна фраза на оба входа, и она английская: консольный
   * вход печатает ее рядом с английскими же причинами. Окно русское целиком, а
   * отмена – единственный исход, который обычный читатель видит на обычном
   * пути, и оставлять ему английскую фразу незачем. Поэтому фразу для него
   * составляет окно. Фаза при этом берется с провода, так что собственного
   * знания о том, что и в каком порядке делает ядро, у окна не появляется.
   *
   ...
   *
   * Прочие отказы по-прежнему показывают фразу ядра как есть – это отдельная
   * незакрытая позиция.
   */
...
<main>
  <div class="ready">
    ...
    {:else if screen.kind === 'absent'}
      <p class="about">
        Программа возьмет TLHdig с Zenodo, разложит корпус по папкам в загрузках и создаст опись –
        один файл HTML, который открывается в браузере без сети. Это занимает от нескольких секунд
        до минуты с небольшим.
      </p>
