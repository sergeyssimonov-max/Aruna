  ...
  <div class="ready">
    ...
    {:else if screen.kind === 'present'}
      ...
      <!--
        ...
        две – «прочитаны все» и «столько-то не будут преобразованы».
        ...
        Крупным числом стоит одно – непрочитанные: у него единственного есть
        последствие для владельца, эти документы не попадут в PDF. Два других
        отвечают на другие вопросы – что говорит стандарт XML и что говорят
        пространства имен, – и потому идут ниже обычным текстом, как сведения
        о данных. Абзаца с ними нет, когда манифест второй половины не несет:
        пакет, собранный до 10.09.2026, ее не считал, – и нет, когда считал, а
        сказать по ней оказалось нечего.

        Четыре и тринадцать взяты из `limits` по ключу, а не вычитанием одного
        итога из другого: разность совпала бы с ними числом, но назвалась бы
        при этом классом, которого в манифесте не спрашивали.
      -->
      {#if screen.markup}
        {@const rawLessThan =
        ...
        <div class="markup">
          ...
          {:else}
            ...
            <p class="markup-about">
              Программа их не читает: разметка нарушена внутри текста – не сходятся теги, пропущен
              знак «=» в атрибуте, значение атрибута не закрыто до конца файла. Все они лежат в
              пакете вместе с остальными и открываются как обычно – такая разметка мешает переводу в
              PDF, а не хранению. Поименный список с причинами и номерами строк – в файле
              manifest.json, раздел xml.
            </p>
            {#if screen.markup.not_well_formed_xml > 0 && (rawLessThan > 0 || colonWithoutLocalName > 0)}
              <p class="markup-about">
                {#if rawLessThan > 0}Правила XML нарушают
                  {spaced(screen.markup.not_well_formed_xml)} – те же {spaced(screen.markup.unread)}
                  и еще {spaced(rawLessThan)}, которые программа принимает, хотя стандарт их
                  запрещает: внутри значения атрибута стоит голый знак «меньше».{/if}
                {#if colonWithoutLocalName > 0}Правила пространств имен нарушают еще
                  {spaced(colonWithoutLocalName)}: как XML они корректны, недопустимо имя элемента –
                  у имени вида &lt;AO:-…&gt; нет части после двоеточия.{/if}
              </p>
            {/if}
          {/if}
        </div>
