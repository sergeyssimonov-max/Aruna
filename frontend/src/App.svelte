  ...
  <div class="ready">
    ...
    {:else if screen.kind === 'present'}
      ...
      <!--
        ...
        на вопрос «что собрано», а эти две – на вопрос «как оно устроено»,
      ...
      {/if}
      <!--
        Что разборщик сказал о документах – в своей обертке, хотя строка в ней
        теперь одна: класс `.markup` отделяет счетчик разметки от итогов выше,
        и на него опираются тесты. Веток у строки по-прежнему две – «нет ни
        одной» и «столько-то из стольких-то», – и обертка держит их одним
        сообщением.
      ...
      {#if screen.markup}
        <div class="markup">
          {#if screen.markup.not_well_formed === 0}
            <p class="spread">
              <span>
                Некорректных XML нет – проверено
...
-->
<style>
  /* Колонка при одном ребенке ничего не двигает – и стоит нарочно.
   *
   * Сама обертка сжата по содержимому: она флекс-элемент `.ready`, а внутри у
   * нее одна строка, так что обычный блок дал бы то же до пикселя. Промежутка
   * тут больше нет – он делил строки одного сообщения, а сообщение теперь в
   * одну строку. Три объявления оставлены на случай второй строки: та встанет
   * по центру под первой, а не по левому краю той, что окажется шире.
   *
   ...
   * группой сверху вниз сами: два итога пятнадцатым размером, под ними
   * разбивка тринадцатым, под ней разметка. Сдвинуть их плотнее отсюда нечем
   * и не нужно: `.metrics` и `.spread` объявлены в `app.css`, а те же классы
   * носят строки прогона и отчета, и правило в этом файле переставило бы
   * заодно и их – ради экрана, о котором его не просили. */
  .markup {
    display: flex;
