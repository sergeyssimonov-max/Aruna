  ...
  <div class="ready">
    ...
    {:else if screen.kind === 'present'}
      ...
      <!--
        ...
        три – «прочитаны все», «столько-то не будут преобразованы» и, для
        пакета без счета модели, «программа не прочитала столько-то».
        ...
        принадлежащим 223. Три части под счетчиком – объяснение, и каждая
        отвечает на свой вопрос: что не читает разборщик, что нарушает XML,
        но разборщиком принимается, и что корректно как XML, но нарушает
        пространства имен. Первая взята из `unread`, две другие – из `limits`
        по ключу, а не вычитанием одного итога из другого: разность совпала бы
        с ними числом, но назвалась бы при этом классом, которого в манифесте
        не спрашивали. Части с нулем не показываются.
      ...
      {#if screen.markup}
        {@const rawLessThan =
          screen.markup.limits.find((count) => count.limit === 'raw-less-than-in-attribute-value')
            ?.documents ?? 0}
        {@const colonWithoutLocalName =
          screen.markup.limits.find((count) => count.limit === 'colon-without-local-name')
            ?.documents ?? 0}
        <div class="markup">
          {#if screen.markup.objected_to > 0}
            ...
            <ul class="markup-about markup-parts">
              {#if screen.markup.unread > 0}
                <li>
                  {spaced(screen.markup.unread)} не читает разборщик: правила XML нарушены внутри тела
                  документа – не сходятся теги, пропущен знак «=» в атрибуте, значение атрибута не закрыто
                  до конца файла.
                </li>
              {/if}
              {#if rawLessThan > 0}
                <li>
                  {spaced(rawLessThan)} нарушают правила XML, но разборщик их принимает – известное ограничение:
                  внутри значения атрибута стоит голый знак «меньше».
                </li>
              {/if}
              {#if colonWithoutLocalName > 0}
                <li>
                  {spaced(colonWithoutLocalName)} корректны как XML, но нарушают правила пространств имен:
                  у имени элемента вида &lt;AO:-…&gt; нет части после двоеточия.
                </li>
              {/if}
            </ul>
            <p class="markup-about">
              Все они лежат в пакете вместе с остальными и открываются как обычно – такая разметка
              мешает переводу в PDF, а не хранению. Поименный список с причинами и номерами строк –
              в файле manifest.json, раздел xml.
            </p>
          ...
          {:else}
            ...
            <p class="markup-about">
              ...
              остальными и открываются как обычно. Поименный список с причинами и номерами строк – в
              файле manifest.json, раздел xml.
            </p>
          {/if}
        </div>
      {/if}
...
<style>
  ...
  /* Три части объяснения – список, а не абзац: каждая отвечает на свой
   * вопрос, и отдельный элемент не дает предложениям слипнуться. Маркеров и
   * отступа нет, чтобы список читался теми же строками, что `.markup-about`
   * рядом; промежуток между частями меньше, чем между абзацами блока. */
  .markup-parts {
    display: flex;
    flex-direction: column;
    gap: 6px;
