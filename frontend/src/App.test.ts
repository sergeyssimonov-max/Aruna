...
describe('пакет есть', () => {
  ...
  it('показывает ровно пять чисел и ничего больше', async () => {
    ...
    expect(Array.from(container.querySelectorAll('.metrics, .spread')).map(plain)).toEqual([
      ...
      'Не будут преобразованы – 223 из 23 936',
    ])
  })
  ...
  it('показывает, сколько документов не будут преобразованы, и что они в пакете', async () => {
    ...
    expect(await screen.findByText(/Не будут преобразованы/)).toBeInTheDocument()
   ...
   * сказать три вещи: беда в разметке исходных документов, документы на месте,
   * поименный список лежит в манифесте. Проверяется по смыслу – по трем
   * оборотам, которые нельзя убрать, не убрав саму мысль.
   */
  it('объясняет, что за документы и где их искать', async () => {
    const container = await overPackage()
    await screen.findByText(/Не будут преобразованы/)
    ...
    expect(about).toContain('manifest.json')
    // Откуда складывается 223: три составляющие, каждая со своим вопросом, и
    // числа с провода, а не вписанные в шаблон. Каждая – своим элементом
    // списка: `{#if}` внутри одного абзаца легко склеивает предложения, а
    // приведение пробелов в этой проверке склейку бы и спрятало.
    const parts = Array.from(container.querySelectorAll('.markup-parts li')).map((node) =>
      (node.textContent ?? '').replace(/\s+/g, ' ').trim(),
    )
    expect(parts).toHaveLength(3)
    expect(parts[0]).toContain(
      '206 не читает разборщик: правила XML нарушены внутри тела документа',
    )
    expect(parts[1]).toContain(
      '4 нарушают правила XML, но разборщик их принимает – известное ограничение',
    )
    expect(parts[1]).toContain('голый знак «меньше»')
    expect(parts[2]).toContain('13 корректны как XML, но нарушают правила пространств имен')
    expect(parts[2]).toContain('<AO:-…>')
    // 210 – ответ на вопрос о стандарте XML, а не часть 223: на экране его нет.
    expect(about).not.toContain('210')
  })

  /**
   * **Тринадцать названы корректным XML, и это проверка смысла, а не слов.**
   *
   * Они законны по XML 1.0 – двоеточие и дефис входят в состав имени, – и
   * нарушают «Пространства имен в XML», отдельный стандарт. Манифест и
   * `docs/XML-CONTRACT.md` говорят это словами; сказать на экране иначе значило
   * бы противоречить собственному пакету. Проверяется здесь потому, что убрать
   * эту половину фразы легко и незаметно, а без нее 13 читаются как еще одна
   * порция испорченных документов.
   *
   * Заодно – последняя проверка того, что словарь разработчика с экрана ушел:
   * «строгий разбор» человеку говорит, будто есть настройка, которую можно
   * подкрутить, и с 13.09.2026 этих слов в окне нет.
   */
  it('называет тринадцать документов корректным XML и не говорит о строгости', async () => {
    const container = await overPackage()
    await screen.findByText(/Не будут преобразованы/)

    expect(aboutText(container)).toContain('13 корректны как XML')
  ...
  it('без числа модели показывает непрочитанные под их вопросом', async () => {
    ...
    expect(screen.queryByText(/Не будут преобразованы/)).toBeNull()
