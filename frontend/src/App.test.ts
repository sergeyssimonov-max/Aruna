...
describe('пакет есть', () => {
  ...
  /**
   * **Сводка о разметке видна, и ни одно слово не говорит о судьбе документов.**
   *
   * Фраза о том, что все 206 лежат в пакете, стояла тут же и снята 06.09.2026
   * вместе с остальным: на экране оставлены пять чисел. Утверждать обратное –
   * что документы выброшены – экран по-прежнему не вправе, и это единственное,
   * что здесь можно проверить словами.
   */
  it('показывает, сколько документов не корректный XML', async () => {
    await overPackage()
