...
const markup: XmlSummary = {
  ...
  // разборщик ядра принимает, а полностью строгий – нет.
...
describe('пакет есть', () => {
  ...
  /**
   * **На экране пакета ровно шесть чисел, и это решение, а не то, что вышло.**
   *
   ...
   *
   * Шестое добавлено 10.09.2026 решением владельца вместе со второй половиной
   * замера: 206 – это отказы разборщика ядра, а 223 – счет полностью строгого,
   * и без второго числа первое читается как весь ответ. Оно и последнее: любое
   * седьмое возвращает экран туда, откуда его в тот раз убирали.
   *
   ...
   */
  it('показывает ровно шесть чисел и ничего больше', async () => {
    const container = await overPackage(STATS_SAMPLE.manifest)
    await screen.findByText(/Самая большая группа/)
    ...
    expect(Array.from(container.querySelectorAll('.metrics, .spread')).map(plain)).toEqual([
      ...
      'Полностью строгий разбор насчитывает 223',
    ])
  })
  ...
  it('объясняет, что за документы и где их искать', async () => {
    const container = await overPackage()
    await screen.findByText(/Некорректных XML/)

    const about = container.querySelector('.markup-about')?.textContent?.replace(/\s+/g, ' ')

    expect(about).toContain('в разметке самих документов')
    expect(about).toContain('лежат в пакете вместе с остальными')
    expect(about).toContain('manifest.json')
    // Вторая половина названа числом с провода и описана словами: два класса,
    // и ни один из них не число в шаблоне.
    // Со швом блока: `{#if}` внутри абзаца легко склеивает предложения, а
    // приведение пробелов в этой проверке склейку бы и спрятало.
    expect(about).toContain('файла. Еще 17 документов')
    expect(about).toContain('голый знак «<»')
  ...
  it('без второй половины показывает только первую', async () => {
    ...
    expect(screen.queryByText(/Полностью строгий разбор/)).toBeNull()

    const about = container.querySelector('.markup-about')?.textContent?.replace(/\s+/g, ' ')
    expect(about).toContain('лежат в пакете вместе с остальными')
    expect(about).not.toContain('Еще')
  })
