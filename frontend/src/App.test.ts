...
describe('пакет есть', () => {
  ...
  it('показывает ровно пять чисел и ничего больше', async () => {
    ...
    expect(Array.from(container.querySelectorAll('.metrics, .spread')).map(plain)).toEqual([
      ...
      'Не будут преобразованы – 206 из 23 936',
    ])
  })
  ...
  it('объясняет, что за документы и где их искать', async () => {
    ...
    expect(about).toContain('разметка нарушена внутри текста')
    expect(about).toContain('лежат в пакете вместе с остальными')
    expect(about).toContain('manifest.json')
    // Вторая половина названа числами с провода и описана словами: два класса,
    // каждый со своим стандартом, и ни одно из чисел не вписано в шаблон.
    // Со швом блока: `{#if}` внутри абзаца легко склеивает предложения, а
    // приведение пробелов в этой проверке склейку бы и спрятало.
    expect(about).toContain('Правила XML нарушают 210 – те же 206 и еще 4')
    expect(about).toContain('голый знак «меньше»')
    expect(about).toContain('Правила пространств имен нарушают еще 13')
    expect(about).toContain('<AO:-…>')
  })
  ...
  it('называет тринадцать документов корректным XML и не говорит о строгости', async () => {
    ...
    expect(aboutText(container)).toContain('как XML они корректны')
  ...
  /**
   * **Пакет, собранный между 06.09 и 10.09.2026, второй половины не показывает.**
   *
   * Манифест ее не несет, окно нулей не печатает: «еще 0 документов» и строка
   * «насчитывает 0» сказали бы о корпусе неправду. Первая половина при этом на
   * месте целиком.
   */
  it('без второй половины показывает только первую', async () => {
    const container = await overPackage(bare, markupWithoutLimits)
    await screen.findByText(/Не будут преобразованы/)

    const about = aboutText(container)
    expect(about).toContain('лежат в пакете вместе с остальными')
    // Абзаца о стандартах нет вовсе: «правила XML нарушают 0» сказало бы о
    // корпусе неправду, а пакет старше 10.09.2026 этого просто не считал.
    expect(about).not.toContain('Правила XML нарушают')
