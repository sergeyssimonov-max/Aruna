...
const markup: XmlSummary = {
  ...
  // Вторая половина того же вопроса, с 10.09.2026: документы, которые
  // программа читает, а стандарты их не допускают – четыре нарушают XML,
  // тринадцать нарушают пространства имен, оставаясь корректным XML.
  beyond_this_parser: 17,
  limits: [
    { limit: 'raw-less-than-in-attribute-value', documents: 4 },
    { limit: 'colon-without-local-name', documents: 13 },
  ],
  documents_beyond_this_parser: [
    {
      file: 'CTH 211/DAAM 6.93.xml',
      limit: 'raw-less-than-in-attribute-value',
      line: 8,
      column: 47,
    },
  ],
  not_well_formed_xml: 210,
  objected_to: 223,
}

/** Тот же пакет, но собранный до 10.09.2026: второй половины манифест не несет. */
const markupWithoutLimits: XmlSummary = {
  ...markup,
  beyond_this_parser: 0,
  limits: [],
  documents_beyond_this_parser: [],
  not_well_formed_xml: 0,
  objected_to: 0,
}
...
/**
 * Пояснения о разметке одной строкой.
 *
 * Абзацев с 13.09.2026 два – о последствии и о том, что говорят стандарты, – и
 * проверка, читающая только первый, молча перестала бы видеть второй.
 */
function aboutText(container: HTMLElement): string {
  return Array.from(container.querySelectorAll('.markup-about'))
...
describe('пакет есть', () => {
   ...
   *
   * 06.09.2026 их было тринадцать: к пяти тогдашним прибавлялись рукописи без
   ...
   *
   * Шестым с 10.09.2026 стояло 223, и 13.09.2026 оно с экрана снято. Не потому,
   * что чисел стало много, а потому, что подпись под ним была неверна: она
   * называла 206 «некорректными XML», а 223 – счетом «полностью строгого
   * разбора», то есть три разных вопроса выстраивала в одну шкалу строгости,
   * которой нет. Крупным осталось одно число – непрочитанные, у которого
   * единственного есть последствие для человека; 210 и 13 ушли в пояснение
   * обычным текстом, как число 17 стояло там и раньше. Шестое возвращается
   * только вместе с вопросом, на который оно отвечает, а не как счетчик.
   *
   * Пятое число с 13.09.2026 – 223, а не 206. Последствие «не будут
   * преобразованы» принадлежит множеству, которое отвергает модель документа,
   * а 206 – только отказы разборщика: подпись обещала читателю последствие
   * чужого числа.
   *
  ...
  it('показывает ровно пять чисел и ничего больше', async () => {
    ...
    expect(Array.from(container.querySelectorAll('.metrics, .spread')).map(plain)).toEqual([
      ...
      'Не могут быть преобразованы в PDF – 223 из 23 936',
    ])
  })
   ...
   * уйти с мыслью, будто 223 документа из пакета выброшены: они там, и экран
   ...
   * поэтому: снять ее легко и незаметно, а без нее `223` читается как
   * «223 выброшено».
   */
  it('показывает, сколько документов не будут преобразованы, и что они в пакете', async () => {
    await overPackage()

    expect(corpusXml).toHaveBeenCalledWith(PACKAGE)
    expect(await screen.findByText(/Не могут быть преобразованы/)).toBeInTheDocument()
  ...
  /**
   * **Экран объясняет, в чем беда, а не только сколько ее.**
   *
   * Числа без объяснения читаются как приговор пакету. Пояснение обязано
   * сказать две вещи: беда в разметке исходных документов и документы на
   * месте. Проверяется по смыслу – по оборотам, которые нельзя убрать, не убрав
   * саму мысль.
   *
   * До 17.09.2026 пояснение говорило и третье: из чего складывается 223 – три
   * составляющие, каждая со своим вопросом, – и где лежит поименный список.
   * Решением владельца это с экрана снято; разбор лежит в манифесте, и экран
   * отвечает на вопрос «что получилось», а не «из чего это сложилось».
   */
  it('объясняет, что за документы', async () => {
    const container = await overPackage()
    await screen.findByText(/Не могут быть преобразованы/)

    const about = aboutText(container)

    expect(about).toContain('Программа их не читает')
    expect(about).toContain('лежат в пакете вместе с остальными')
    // Составляющих на экране нет – ни списка, ни его чисел, ни отсылки к
    // манифесту, которая была при них.
    expect(container.querySelectorAll('.markup-parts')).toHaveLength(0)
    expect(about).not.toContain('206')
    expect(about).not.toContain('manifest.json')
    // 210 – ответ на вопрос о стандарте XML, а не часть 223: на экране его нет.
    expect(about).not.toContain('210')
  })

  /**
   * **Словарь разработчика с экрана ушел.**
   *
   * «Строгий разбор» человеку говорит, будто есть настройка, которую можно
   * подкрутить, а словом «некорректный» нельзя называть 223: тринадцать из них
   * законны по XML 1.0 и нарушают «Пространства имен в XML», отдельный
   * стандарт. Так говорят манифест и `docs/XML-CONTRACT.md`, и сказать на
   * экране иначе значило бы противоречить собственному пакету.
   *
   * Прежде тест держал и вторую половину – фразу «13 корректны как XML» в
   * пояснении. Составляющие сняты с экрана 17.09.2026 решением владельца, и
   * фраза ушла вместе с ними: экран больше не говорит об этих тринадцати
   * ничего, ни хорошего, ни дурного.
   */
  it('не говорит о строгости и не называет документы некорректными', async () => {
    await overPackage()
    await screen.findByText(/Не могут быть преобразованы/)

    const shown = (document.body.textContent ?? '').toLowerCase()
    for (const word of ['строг', 'некорректн']) {
      expect(shown).not.toContain(word)
    }
  })

  /**
   * **Пакет, собранный между 06.09 и 10.09.2026, последствия не заявляет.**
   *
   * Манифест такого пакета числа модели не несет – только отказы разборщика.
   * Поставить под ними «не будут преобразованы» значило бы повторить ту же
   * ошибку привязки: модель отвергает больше, а сколько – этот пакет не знает.
   * Поэтому 206 стоит под своим собственным вопросом, без PDF, и составляющих
   * нет: «еще 0 документов» сказало бы о корпусе неправду.
   *
   * Отсылка к `manifest.json` снята отсюда 18.09.2026 решением владельца – тем
   * же, каким она ушла из главной ветки сутками раньше. Цена названа: читатель
   * пакета, собранного между 06.09 и 10.09.2026, ниоткуда на экране не узнает,
   * что поименный список с причинами и номерами строк лежит в манифесте. Сам
   * список не тронут, и экран нигде больше манифеста не называет.
   */
  it('без числа модели показывает непрочитанные под их вопросом', async () => {
    const container = await overPackage(bare, markupWithoutLimits)
    await screen.findByText(/Программа не прочитала/)

    const row = container.querySelector('.markup .spread')?.textContent?.replace(/\s+/g, ' ').trim()
    expect(row).toBe('Программа не прочитала – 206 из 23 936')
    expect(screen.queryByText(/Не могут быть преобразованы/)).toBeNull()

    const about = aboutText(container)
    expect(about).toContain('лежат в пакете вместе с остальными')
    expect(about).not.toContain('PDF')
    expect(about).not.toContain('пространств имен')
    expect(about).not.toContain('manifest.json')
    expect(container.querySelectorAll('.markup-about')).toHaveLength(1)
  })

  /**
   * **Крупное число – число модели, а не разборщика.**
   *
   * Числа нарочно не настоящие и не связаны суммой: окно, взявшее крупным
   * `unread` или сложившее составляющие само, покажет здесь не то. До
   * 13.09.2026 оно так и делало – показывало 206 с последствием, которое
   * принадлежит 223.
   */
  it('ставит крупным то, что отвергает модель', async () => {
    const container = await overPackage(bare, { ...markup, unread: 5, objected_to: 40 })
    await screen.findByText(/Не могут быть преобразованы/)

    const big = Array.from(container.querySelectorAll('.markup .count')).map((node) =>
      node.textContent?.trim(),
    )
    expect(big).toEqual(['40'])
  })
...
describe('кончилось', () => {
  ...
  /**
   * **Трем кодам подробность нужна, и она приходит второй строкой.**
   *
   * `collision` называет два исходных документа, `folder_collision` – две
   * группы и первый документ каждой, `archive_duplicate` – имя записи, и
   * больше этих имен читателю взять неоткуда: разобраться он может только по
   * ним. Остальным кодам вторая строка не положена – у них фраза ядра ничего
   * не добавляет к сказанному по-русски.
   */
  it.each([
    [
      'collision',
  ...
  it.each([
    ...
    // Прежнего пакета может не быть – первая сборка или новая папка, – и
    // окно этого не знает; безусловное «остался на месте» было неправдой
    // (заслон 26.09.2026).
    expect(screen.queryByText(/прежний пакет остался/i)).toBeNull()
  ...
  it('на отмене без фазы дает запасное пояснение', async () => {
    ...
    expect(
      await screen.findByText(
        /Остановлено\. Незаконченное убрано, а прежний пакет, если он был, остался на месте\./,
      ),
    ).toBeInTheDocument()
