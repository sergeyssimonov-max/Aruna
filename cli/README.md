...
обработчика сигнала у неё нет. Внутри всё для остановки готово — работа
...
Что после этого остаётся в `~/Downloads`: каталог `.TLHdig_Beta_0.3.build.<pid>.<n>`
— незавершённая сборка убитого прогона. Он скрытый (с точки), никуда не
