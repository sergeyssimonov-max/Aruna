...
mod tests {
    ...
    ///
    /// `empty.xml` не назван: он пуст и отвечает на вопрос «корректный ли это
    /// XML» раньше разбора. `not-utf8.xml` стоял рядом с ним по соседней
    /// причине – «первый не текст», – и 13.09.2026 она перестала быть правдой:
    /// у байтов не в UTF-8 появилась своя причина, и ответ на них теперь
    /// закреплен здесь, а не подразумевается. Замер 16.09.2026: до этой строки
    /// ответ фикстуры не держал ни один тест.
    #[test]
    fn the_fixtures_get_the_answers_they_were_written_for() {
        let dir = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("fixtures/xml");
        let cases: [(&str, Option<Reason>); 7] = [
            (
            ...
            // `FF FE` в начале – метка порядка байтов UTF-16, в UTF-8 не
            // встречающаяся никогда. Разметка после нее правильная, и
            // единственное, что с документом не так, – кодировка.
            ("malformed/not-utf8.xml", Some(Reason::InvalidEncoding)),
