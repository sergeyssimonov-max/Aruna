...
**Amended 2026-09-26: text is compared code point for code point, and no step
normalises it — not NFC, not NFD, not NFKC — on either side of the comparison.**
The measurement, over the 23 936 documents of the 2.6.1 package on 2026-09-26
(Python 3.9.6, Unicode 13.0): **2 127 documents are not in NFC**. 2 077 of them
are not in NFC for one reason only, `U+2329`/`U+232A` — 16 735 occurrences in
2 089 documents — which have singleton canonical decompositions to
`U+3008`/`U+3009`: NFC, NFD and NFKC all replace the first pair with the second.
The other 50 carry base letters followed by combining marks (`u` + `U+0308`,
`C` + `U+0327`, `S` + `U+0323`, …), which NFC would compose. Either way a
normalising step changes code points the source wrote, and a comparison that
normalises one side, or both, reports those documents as intact when the PDF
carries other characters than the XML. The same rule is why the refusal of two
CTH folders that differ only in Unicode form names their code points rather
than their printed names (`a0f929c`): what prints the same is not the same.
